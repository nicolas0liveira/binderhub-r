...
#View(GSPC$GSPC.Close)
head(BVSP)

GSPC <- na.omit(BVSP)

retorno <- diff(log(Cl(GSPC$GSPC.Close)))
retorno <- na.omit(retorno)
View(retorno)

hist(retorno$GSPC.Close)

ggplot(retorno, aes(x=GSPC.Close)) +
  geom_histogram()


mean_return <- mean(retorno$GSPC.Close, na.rm=TRUE) 
mean_return
...
sd(retorno$GSPC.Close, na.rm = TRUE) #antilog -1

exp((sd(retorno$GSPC.Close, na.rm = TRUE)^252)-1)*100
